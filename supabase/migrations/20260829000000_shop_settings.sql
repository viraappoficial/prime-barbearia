...
--
-- Hoje a janela de funcionamento e a duração do slot são constantes do
-- frontend (HORARIOS, SLOT_MIN em domain/agenda.ts). A partir da fatia de
-- escrita de agenda, a RPC public_day_availability e a book_appointment
-- precisam da MESMA regra, no servidor. Esta tabela é a fonte única.
--
-- Configuração ativa única e inequívoca: linha única forçada por
-- `check (id = 1)` + PK. Sem policy de DELETE em lugar nenhum -> ninguém
-- apaga a config; a RPC lê sempre `where id = 1`, sem escolha ambígua.
-- Escrita só admin.
--
-- Rollback: drop table public.shop_settings;
-- Impacto: nenhum no legado (não lê esta tabela). Cria a base para as RPCs.
...
-- seed = os valores que já valiam em domain/agenda.ts (nada novo inventado):
--   Seg-Sex 09:00-20:00, Sáb 09:00-18:00, Dom fechado; slot 45 min; janela 10 dias.
insert into public.shop_settings (id, slot_min, open_hours, max_advance_days) values (
  1,
  45,
