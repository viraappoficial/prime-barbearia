-- ST-3c.1 — `staff_crm_ficha` ganha os blocos `brinde` e `indicacao`.
--
-- Ambos só existem pra CONTA real (`v_cid is not null`); walk-in sem conta
-- devolve os dois como `null` (mesmo comportamento do legado). Visível a
-- todo papel (mesma decisão de D-ST3b5-2 pro plano).
--
-- Resto da função IDÊNTICO a `20260912000200_staff_crm_ficha_plano.sql` —
-- só ganha os 2 blocos novos, reusando `v_visitas` já calculado (contagem
-- de `appointments` concluídos — não `sales.client_name`, ver
-- `20260912000400_staff_crm_relacionamento.sql`).
--
-- Rollback: reaplicar `20260912000200_staff_crm_ficha_plano.sql`.
...
begin
  ...
  if v_cid is not null then
    select jsonb_build_object('nome', coalesce(c.name, c.email, 'Cliente'), 'mimo_dado', r.referrer_gift_given)
    into v_indicado_por
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      'nome', coalesce(c.name, c.email, 'Cliente'), 'mimo_dado', r.referred_gift_given
