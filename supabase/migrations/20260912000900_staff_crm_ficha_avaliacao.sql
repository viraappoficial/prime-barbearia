-- ST-3c.3 — `staff_crm_ficha` ganha o bloco `avaliacoes`: os últimos
-- atendimentos concluídos avaliáveis (id, data, serviços, nota/comentário
-- atuais se já avaliado). Escopado igual ao histórico: barbeiro só vê os
-- PRÓPRIOS atendimentos (a.barber_id = v_uid); admin/vendas vê todos os do
-- cliente. Necessário pra UI listar atendimentos específicos em vez de
-- assumir "o mais recente" (mesma lição do fix de indicação).
--
-- Resto da função IDÊNTICO a `20260912000700_staff_crm_ficha_indicacao_fix.sql`.
-- Rollback: reaplicar `20260912000700_staff_crm_ficha_indicacao_fix.sql`.
...
begin
  ...
  -- ── avaliação privada (ST-3c.3) — últimos atendimentos concluídos
  -- avaliáveis por ESTE staff (barbeiro só os próprios; admin/vendas livre)
  select coalesce(jsonb_agg(jsonb_build_object(
    'appt_id', a.id, 'day', a.day, 'time', a.time, 'services', a.services,
    'barbeiro_nome', (select b.name from public.barbers b where b.id = a.barber_id),
    'nota', a.client_rating, 'comentario', a.client_rating_comment
  ) order by a.day desc, a.time desc), '[]'::jsonb)
  into v_avaliacoes
  from (
    select a.*
    from public.appointments a
